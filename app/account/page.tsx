...
import DailyAttunement from "../../components/DailyAttunement"; // 👈 Integrated the manga-sketch check-in component
...
export default function AccountPage() {
  ...
  if (loading) return <div className="min-h-screen bg-void flex items-center justify-center text-blue-500 font-mono animate-pulse">INITIALIZING HUD...</div>;
  ...
  return (
    <div className="min-h-screen bg-void text-white p-4 md:p-8 font-sans relative overflow-hidden">
      
      {/* BACKGROUND GRID MATRICES */}
      <div className="absolute inset-0 bg-[linear-gradient(rgba(30,24,45,0.15)_1px,transparent_1px),linear-gradient(90deg,rgba(30,24,45,0.15)_1px,transparent_1px)] bg-size-[40px_40px] opacity-30 pointer-events-none"></div>
      ...
      <div className="max-w-6xl mx-auto flex items-center justify-between mb-8 relative z-10">
        ...
        <h1 className="text-xl font-bold uppercase tracking-widest text-transparent bg-clip-text bg-linear-to-r from-gray-200 to-gray-500">
            Command Center
        </h1>
      </div>
      ...
      <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8 relative z-10">
        ...
        <div className="lg:col-span-1 space-y-6">
            ...
            {/* 1. IDENTITY CARD */}
            <div className="bg-void-surface border border-void-border p-6 rounded-3xl shadow-2xl relative overflow-hidden group">
                <div className={`absolute top-0 left-0 w-full h-1 ${barColor} shadow-[0_0_20px_currentColor] opacity-50`}></div>

                <div className="flex flex-col items-center text-center">
                    <div className={`w-24 h-24 rounded-full border-4 border-void-border flex items-center justify-center text-4xl font-bold mb-4 shadow-lg ${barColor} text-white`}>
                ...
                <div className="mt-8">
                    ...
                    <div className="h-3 w-full bg-void rounded-full overflow-hidden border border-void-border">
                        <div 
            ...
            <div className="grid grid-cols-2 gap-4">
                <div className="bg-void-surface border border-void-border p-4 rounded-2xl flex flex-col items-center justify-center hover:border-blue-500/50 transition-colors">
                ...
                <div className="bg-void-surface border border-void-border p-4 rounded-2xl flex flex-col items-center justify-center hover:border-green-500/50 transition-colors">
            ...
            <button 
                ...
                className="w-full py-4 bg-red-900/10 border border-red-900/30 text-red-500 rounded-xl hover:bg-red-900/30 hover:text-white transition flex items-center justify-center gap-2 font-bold text-sm uppercase tracking-wider group cursor-pointer"
            >
                <LogOut size={16} className="group-hover:-translate-x-1 transition-transform" /> Disconnect System
            </button>
        </div>

        {/* --- RIGHT COLUMN: ATTUNEMENT MATRIX & SAVED FILES --- */}
        <div className="lg:col-span-2 space-y-6">
            
            {/* INJECTED SERIALIZATION ENGINE MAP */}
            <DailyAttunement />

            <div className="bg-void-surface border border-void-border rounded-3xl p-6 h-full min-h-100">
                ...
                {bookmarks.length === 0 ? (
                    <div className="h-64 border-2 border-dashed border-void-border rounded-2xl flex flex-col items-center justify-center text-center p-8 opacity-50">
                        ...
                        <Link href="/read" className="px-6 py-2 bg-blue-600 rounded-full text-sm font-bold hover:bg-blue-500 transition-colors">
                            Start Reading
                        </Link>
                    </div>
                ) : (
                    <div className="grid gap-4">
                        {bookmarks.map((b) => (
                            <div key={b.id} className="group flex items-center justify-between bg-void border border-void-border p-4 rounded-xl hover:border-blue-500/50 hover:bg-void-surface transition-all cursor-pointer relative overflow-hidden">
                                ...
                                >
                                    ...
                                    <div>
                                        <h4 className="font-bold text-white text-lg group-hover:text-blue-400 transition-colors">{b.title}</h4>
                                ...
                                <button 
                                    className="p-3 text-gray-600 hover:text-red-500 hover:bg-red-900/20 rounded-lg transition opacity-0 group-hover:opacity-100 cursor-pointer"
